...
def print_employee(e: Dict) -> None:
    print(
        f"ID: {e.get('id')}\tName: {e.get('first_name')} {e.get('last_name')}\tAge: {e.get('age')}\tPosition: {e.get('position')}\tEmail: {e.get('email')}\tPhone: {e.get('phone')}"
    )
...
def input_int(
    prompt: str, default: Optional[int] = None, allow_empty=False
) -> Optional[int]:
    while True:
...
def search_by_last_name(employees: List[Dict]) -> List[Dict]:
    print("\nSearch by last name")
    name = input_nonempty(
        "Enter last name (or substring) to search (case-insensitive): "
    )
...
def filter_by_lastname_initial(employees: List[Dict]) -> List[Dict]:
    ...
    while True:
        s = input(
            "Enter a single letter (last-name should begin with this letter): "
        ).strip()
    ...
    matches = [
        e for e in employees if e.get("last_name", "").lower().startswith(letter)
    ]
    if not matches:
...
def main():
    ...
    file_path = input_nonempty(
        "Enter path to employee JSON file to load (will be created if missing): "
    )
